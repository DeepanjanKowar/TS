...
colnames(tweets.df)[5] <- "start"
timevis(tweets.df)

# tweets.df <- read.csv("jkSentences.csv")
#tweets.df<- read.csv("/Users/Deepanjan/Desktop/R Project/Sample JSON/jallikattu.csv",stringsAsFactors = F,header=F)
...
# noundata.df<-subset(noundata.df, nchar(trim(as.character(noun_phrase)))>2)
# mycorpus2 <- Corpus(VectorSource(noundata.df$noun_phrase))
# remove <- function(x) gsub('\\b\\w{1}\\b','',x)
# mycorpus2<- tm_map(mycorpus2, content_transformer(remove))
# noundata.df <- data.frame(text = sapply(mycorpus2, as.character), stringsAsFactors = F)
noundata.df<-unique(noundata.df)
write.csv(noundata.df, file = "nounfile.csv",row.names = FALSE)

verbdata.df <- read.csv("verbfile.csv", col.names = c("verb_phrase"))
# verbdata.df<-subset(verbdata.df, nchar(trim(as.character(verb_phrase))) >0)
# mycorpus3<- Corpus(VectorSource(verbdata.df$verb_phrase))
# mycorpus3<- tm_map(mycorpus3, content_transformer(remove))
# verbdata.df <- data.frame(text = sapply(mycorpus3, as.character), stringsAsFactors = F)
verbdata.df<-unique(verbdata.df)
write.csv(verbdata.df, file = "verbfile.csv",row.names=FALSE)
...
write.csv(clusters.df, "jallikattu_Clusters.csv")
write.csv(topactors,"jallikattu_Actors.csv")
