shinyServer(function(input, output, session) {
        ...
        sidebarLayout(
          ...
          mainPanel(
            tabsetPanel(
              ...
              tabPanel("Noun Phrase Cloud",plotOutput("plotgraph1")),
              tabPanel("Verb Phrase Cloud",plotOutput("plotgraph2"))
            )
          )
        )
      )
  ...
  observeEvent(input$update1,{
    ...
    output$plotgraph1 <- renderPlot({
        Nouns <- noun_phrase()
        tb<- table(Nouns$noun_phrase)
        wordcloud(names(tb), as.numeric(tb),max.words = 100, random.order = FALSE, min.freq = 1, colors = rainbow(3), scale = c(1,1))
    })
    output$plotgraph2 <- renderPlot({
      Verbs <- verb_phrase()
      tb<- table(Verbs$verb_phrase)
      wordcloud(names(tb), as.numeric(tb),max.words = 100, random.order = FALSE, min.freq = 1, colors = rainbow(3),scale = c(1,1))
    })
  })
